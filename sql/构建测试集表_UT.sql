-- -------------创建副本表------------
-- --创建对应的训练集表
-- if object_id('UTKinectAction3D_copy','U') is not null
--     drop table UTKinectAction3D_copy;
...
--     [HandTipRight],
--     [ThumbRight]
-- INTO UTKinectAction3D_copy
-- FROM UTKinectAction3D;
-- ----------------到这里UT数据集需要先对copy滤波一遍，保持FileName唯一

if object_id('UTKinectAction3D_origin','U') is not null
    drop table UTKinectAction3D_origin;
if object_id('UTKinectAction3D_filtered_len_s','U') is not null
    drop table UTKinectAction3D_filtered_len_s;
SELECT TOP 0 * INTO UTKinectAction3D_origin FROM UTKinectAction3D_copy;
SELECT TOP 0 * INTO UTKinectAction3D_filtered_len_s FROM UTKinectAction3D_copy;


if object_id('UT3DExperimentTest','U') is not null
    drop table UT3DExperimentTest;
-- 创建新表（使用 NVARCHAR(MAX) 替代 TEXT）
CREATE TABLE [dbo].[UT3DExperimentTest] (
...
----创建各比例测试集表
-- 清空测试表数据
TRUNCATE TABLE [dbo].[UT3DExperimentTest];
-- 声明变量
...
FROM [dbo].[UTKinectAction3D_copy]
GROUP BY [type];
-- 打开游标
OPEN action_cursor;
...
BEGIN
    ...
    SELECT @test30 = ISNULL(STUFF((
        ...
        FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 1, ''), '');
    -- 插入结果到测试信息表
    INSERT INTO [dbo].[UT3DExperimentTest]
...
-- 创建测试集20%
IF OBJECT_ID('dbo.UTKinectAction3D_test20', 'U') IS NOT NULL
    DROP TABLE dbo.UTKinectAction3D_test20;
SELECT h.*
INTO UTKinectAction3D_test20
FROM UTKinectAction3D_copy h
JOIN (
   SELECT
...
SELECT
    ...
    COUNT(distinct FileName) AS FileNameCount
FROM
    UTKinectAction3D_filtered_len_s
GROUP BY
    type;


select * from UTKinectAction3D_60_filtered_len_s;
select * from UTKinectAction3D_60_origin;
select * from UTKinectAction3D_origin;
select * from UTKinectAction3D_test20;
