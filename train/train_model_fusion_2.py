...
from torch.optim.lr_scheduler import ReduceLROnPlateau
from thop import profile, clever_format  # 添加thop库用于计算FLOPs和Params

class STGCNTrainer:
    def __init__(self, model,train_data_path, train_label_path, num_nodes, use_residual, batch_size=32, num_epochs=300, lr=0.1,
        ...
        N, C, T, V, M=self.train_data.shape
        ...
        self.calculate_model_complexity(3, T, V, M)

    def calculate_model_complexity(self, C, T, V, M):
        # 创建虚拟输入 (batch_size=1)
        dummy_input = torch.randn(1, C, T, V, M).to(self.device)
        # 计算MACs和Params
        macs, params = profile(self.model, inputs=(dummy_input,), verbose=False)

        # 转换单位 (1 MAC = 2 FLOPs)
        flops = macs * 2
        flops_g = flops / 1e9  # 转换为Giga FLOPs
        params_m = params / 1e6  # 转换为Million Params

        # 打印结果
        print("\n" + "=" * 60)
        print(f"模型复杂度分析 (输入尺寸: [1, {C}, {T}, {V}, {M}])")
        ...
        with open(self.logfile_path , "a") as log:
            log.write(f"输入尺寸: [1, {C}, {T}, {V}, {M}]\n")
            log.write(f"参数量 (Params): {params_m:.2f} M\n")
            log.write(f"计算量 (FLOPs): {flops_g:.2f} G\n\n")
    ...
    def calculate_accuracy(self, data_loader):
        ...
        with torch.no_grad():
            for inputs, labels in data_loader:
                _, _, outputs, _ = self.model(inputs)
