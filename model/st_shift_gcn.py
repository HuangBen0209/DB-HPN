...
class Model(nn.Module):
    def __init__(self, num_class=15, num_nodes=25, num_person=1, graph='graph.ntu_rgb_d.Graph', graph_args=None,
        ...
        self.stgcn_block1 = STGCNBlock(in_channels, 64, kernel_size, 1, residual=False, **kwargs0)
        self.stgcn_block2 = STGCNBlock(64, 64, kernel_size, 1, name="Block1", **kwargs)
        self.stgcn_block3 = STGCNBlock(64, 64, kernel_size, 1, name="Block2", **kwargs)
        self.stgcn_block4 = STGCNBlock(64, 64, kernel_size, 1, name="Block3", **kwargs)
        self.stgcn_block5 = STGCNBlock(64, 128, kernel_size, 2, name="Block4", **kwargs)
        self.stgcn_block6 = STGCNBlock(128, 128, kernel_size, 1, name="Block5", **kwargs)
        self.stgcn_block7 = STGCNBlock(128, 128, kernel_size, 1, name="Block6", **kwargs)
        self.stgcn_block8 = STGCNBlock(128, 256, kernel_size, 2, name="Block7", **kwargs)
        self.stgcn_block9 = STGCNBlock(256, 256, kernel_size, 1, name="Block8", **kwargs)
        self.stgcn_block10 = STGCNBlock(256, 256, kernel_size, 1, name="Block9", **kwargs)
        ...
        self.spp_st = SpatialPyramidPooling([1, 2, 4])
        self.spp_shift = SpatialPyramidPooling([1, 2, 4])

        self.fcc1_st = nn.Linear(512 * (1 * 1 + 2 * 2 + 4 * 4), 2048)
        self.fcc2_st = nn.Linear(2048, 256)

        self.fcc1_shift = nn.Linear(256 * (1 * 1 + 2 * 2 + 4 * 4), 2048)
        self.fcc2_shift = nn.Linear(2048, 256)
    ...
    def forward(self, x):
        ...
        # 加入多尺度池化：
        # x_shiftgcn = self.spp_shift(x_shiftgcn)
        # x_shiftgcn = self.fcc1_shift(x_shiftgcn)
        # x_shiftgcn = self.fcc2_shift(x_shiftgcn)  # c=256
        #
        # x_stgcn = self.spp_st(x_stgcn)
        # x_stgcn = self.fcc1_st(x_stgcn)
        # x_stgcn = self.fcc2_st(x_stgcn)  # c=256

        # N*M,C,T,V
        c_new = x_stgcn.size(1)
        x_stgcn = x_stgcn.view(N, M, c_new, -1)
        x_stgcn = x_stgcn.mean(3).mean(1)
        x_stgcn = self.fc(x_stgcn)

        c_new = x_shiftgcn.size(1)
        x_shiftgcn = x_shiftgcn.view(N, M, c_new, -1)
        x_shiftgcn = x_shiftgcn.mean(3).mean(1)
        x_shiftgcn = self.fc(x_shiftgcn)

        # 可学习的权重融合
        normalized_weights = F.softmax(self.fusion_weights, dim=0)  # 归一化权重
        x = normalized_weights[0] * x_stgcn + normalized_weights[1] * x_shiftgcn  # x:batchsize,num_classes

        return x

        # # 方法0、模块交叉融合
        # x = self.stgcn_block1(x,A)
        # x = self.l2(x)
        # x = self.stgcn_block3(x,A)
        # x = self.l4(x)
        # x = self.stgcn_block5(x, A)
        # x = self.l6(x)
        # x = self.stgcn_block7(x, A)
        # x = self.l8(x)
        # x = self.stgcn_block9(x, A)
        # x = self.l10(x)
        ...
        # for shiftgcn in self.shift_gcn_networks:
        #     x_shiftgcn = shiftgcn(x_shiftgcn)
        # x = torch.cat((x_stgcn, x_shiftgcn), dim=1)  # 在通道级别上进行特征拼接
        # # 加入多尺度池化：
        # # x=self.spp_st(x)
        # # x= self.fcc1_st(x)
        # # x = self.fcc2_st(x) #C=256
        #
        # # N*M,C,T,V
        # c_new = x.size(1)
        # x = x.view(N, M, c_new, -1)
        # x = x.mean(3).mean(1)  # x:C=256
        # return self.fc2(x)
