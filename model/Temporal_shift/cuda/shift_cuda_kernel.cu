...
at::Tensor shift_cuda_forward(
  ...
  int out_len = (input.size(2) + stride - 1) / stride;
  auto output = at::zeros({input.size(0), input.size(1), out_len, input.size(3)}, input.options());

//   auto output = at::zeros({input.size(0), input.size(1), (input.size(2)+1)/stride, input.size(3)}, input.options());
